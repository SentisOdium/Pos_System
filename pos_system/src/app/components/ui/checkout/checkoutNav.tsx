    ...
    return(
        <div className="bg-gray-200/80 w-full  h-[848px] rounded-l-4xl shadow-lg p-5 ">
            ...
            <div className="m-4 h-130 overflow-y-scroll">
                {cart.map(item => (
                    <div key={item.id} className="text-xl mb-5">
                        {item.item} x {item.quantity} = <b>₱{item.price! * item.quantity} </b> 
                        {/* <button className="text-red-600 hover:text-red-700"  onClick={() => handleClearItem(item.id)}>x</button> */}
                    </div>
                ))}
            </div>

                <div className="text-xl ml-5 border-t space-"> Items Total: ₱{subtotal}</div>
                <div className="text-xl ml-5">Delivery Fee: {deliveryFee} (subject to change)</div>
                <div className="text-xl ml-5"><strong>Total: {deliveryFee + subtotal}</strong></div>
                {/* <div><p onClick={handleClearCart}>Clear Cart</p></div> */}
